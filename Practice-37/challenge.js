...
const list = orders.map(element => `<li>${element.name} - ${element.food} - ${element.price}</li>`);
...
const listaOrdenada =  orders.sort((a,b) => b.price - a.price).map(item => `<li>${item.name} - ${item.food} - ${item.price}</li>`);
